  ...
  describe "main application" do
    ...
    describe "GET /magic" do
      ...
      it "works" do
        ...
        assert_equal 200, res.code.to_i
        assert_equal "application/json", res.header["Content-Type"]
        assert_equal '{"data":{"service_b":"42"}}', res.body
      end
    end

    describe "GET /magic-x" do
      MAGIC_X_PATH = URI('http://127.0.0.1:3000/magic-x')

      it "works" do
        res = Net::HTTP.get_response(MAGIC_X_PATH)
        assert_equal 200, res.code.to_i
        assert_equal "application/json", res.header["Content-Type"]
        assert_equal '{"data":{"service_b":{"data":{"main":{"answer":"42"},"extended":{"extra":"88"}}}}}', res.body
      end
    end
  end
